...
VoterUI.prototype.renderVoting = function () {
  ...
  html += '<div class="voter-header"><h1>勝者予想</h1><div class="room-badge">Room: ' + HostUI_escapeHtml(this.viewer.roomCode) + '</div></div>';
  ...
  html += '  <div class="room-tag">観戦者</div>';
  html += '</div>';
  ...
  if (st.champPredictOpen) {
    ...
    if (keys.length === 0) {
      ...
      for (var pi = 0; pi < keys.length; pi++) {
        html += '<div class="player-card" data-player="' + keys[pi] + '"><div class="player-card-name">' + HostUI_escapeHtml(participants[keys[pi]].name) + '</div><div class="player-card-status">この人を予想</div></div>';
      }
    }
    html += '</div>';
